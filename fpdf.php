...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='') {
	if ($this->isUnicodeText($txt)) {
		// If text is Unicode, use the custom CellUnicode method
		$this->CellUnicode($w, $h, strtoupper($txt), $border, $ln, $align, $fill, $link);
		file_put_contents('l.txt','CellUnicode\r\n',FILE_APPEND);
	} else {
		// Otherwise, use the standard cell method
		$this->CellStandard($w, $h, strtoupper($txt), $border, $ln, $align, $fill, $link);
		file_put_contents('l.txt','CellStandard\r\n',FILE_APPEND);
	}
}
...
function CellUnicode($w, $h, $txt, $border=0, $ln=0, $align='', $fill=false, $link='') {
    if ($this->isUnicodeText($txt)) {
        ...
        $this->Image($imagePath, $this->GetX()+1, $this->GetY()+1, $newWidth-2, $newHeight-2);
		// Draw a border if it is set
        if ($border > 0) {
            $this->Rect($this->GetX(), $this->GetY(), $newWidth, $newHeight);
        }
        // Manually move the cell position
        if ($ln > 0) {
            $this->SetY($this->GetY() + $newHeight);
            if ($ln == 1) {
                $this->SetX($this->GetX() + $newWidth);
            }
        }
    } else {
        // Use standard Cell function for non-Unicode text
...
function createTextImage($unicode_text, $desiredWidth, $desiredHeight,$align) {
    ...
    // Adjust dimensions to maintain aspect ratio and include padding
    $width = ($desiredWidth*10) + (2 * $padding);
    $height = ($desiredHeight*10) + (2 * $padding);
...
function CellStandard($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
{
	// Output a cell
	$k = $this->k;
